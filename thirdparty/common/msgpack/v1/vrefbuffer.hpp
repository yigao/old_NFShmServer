...
public:
    ...
    {
        size_t nfirst = (sizeof(iovecpp) < 72/2) ?
            72 / sizeof(iovecpp) : 8;

        iovecpp* array = static_cast<iovecpp*>(::malloc(
            sizeof(iovecpp) * nfirst));
        if(!array) {
            throw std::bad_alloc();
        }
...
public:
    ...
    void append_ref(const char* buf, size_t len)
    {
        if(m_tail == m_end) {
            ...
            iovecpp* nvec = static_cast<iovecpp*>(::realloc(
                m_array, sizeof(iovecpp)*nnext));
            if(!nvec) {
                throw std::bad_alloc();
            }
    ...
    const struct iovecpp* vector() const
    {
        return m_array;
    }
    ...
    void migrate(vrefbuffer* to)
    {
        ...
        if(to->m_tail + nused < m_end) {
            ...
            iovecpp* nvec = static_cast<iovecpp*>(::realloc(
                to->m_array, sizeof(iovecpp)*nnext));
            if(!nvec) {
                ::free(empty);
                throw std::bad_alloc();
            }
        ...
        std::memcpy(to->m_tail, m_array, sizeof(iovecpp)*nused);

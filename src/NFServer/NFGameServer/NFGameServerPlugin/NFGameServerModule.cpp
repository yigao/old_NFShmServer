...
bool NFCGameServerModule::Awake()
{
	...
	if (pConfig)
	{
        NFServerConfig* pConfig = NFConfigMgr::Instance()->GetAppConfig(NF_ST_GAME_SERVER);
        if (pConfig) {
            int64_t unlinkId = NFMessageMgr::Instance()->BindServer(NF_ST_GAME_SERVER, pConfig->mUrl,
            ...
            if (unlinkId >= 0) {
                ...
                */
                ...
                NFMessageMgr::Instance()->AddEventCallBack(NF_ST_GAME_SERVER, gameServerLinkId, this,
                                                           &NFCGameServerModule::OnGameSocketEvent);
                NFMessageMgr::Instance()->AddOtherCallBack(NF_ST_GAME_SERVER, gameServerLinkId, this,
...
int NFCGameServerModule::OnHandleProxyAgentReport(const proto_ff::ServerInfoReport& xData)
{
    ...
    if (pServerData == nullptr)
    {
        ...
        pServerData->mUnlinkId = NFMessageMgr::Instance()->ConnectServer(NF_ST_GAME_SERVER, xData.url(), PACKET_PARSE_TYPE_INTERNAL);
        NFMessageMgr::Instance()->CreateLinkToServer(NF_ST_GAME_SERVER, xData.bus_id(), pServerData->mUnlinkId);

        NFMessageMgr::Instance()->AddEventCallBack(NF_ST_GAME_SERVER, pServerData->mUnlinkId, this, &NFCGameServerModule::OnProxyAgentServerSocketEvent);
        NFMessageMgr::Instance()->AddOtherCallBack(NF_ST_GAME_SERVER, pServerData->mUnlinkId, this, &NFCGameServerModule::OnHandleProxyAgentOtherMessage);
    }
    else {
...
int NFCGameServerModule::OnHandleProxyAgentOtherMessage(uint64_t unLinkId, uint64_t playerId, uint64_t busId, uint32_t nMsgId, const char* msg, uint32_t nLen)
{
    NFLogTrace(NF_LOG_GAME_SERVER_PLUGIN, 0, "-- begin --");
    NFLogWarning(NF_LOG_GAME_SERVER_PLUGIN, playerId, "msg:{} not handled!", nMsgId);
    NFLogTrace(NF_LOG_GAME_SERVER_PLUGIN, 0, "-- end --");

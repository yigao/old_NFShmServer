...
#include "NFMallocChunk.h"

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
NF_SizeMap NF_Static::_sizemap;

size_t NF_SizeMap::AlignmentForSize(size_t size)
{
...
int NF_SizeMap::NumMoveSize(size_t size)
{
	if (size == 0)
		return 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void NF_SpanAllocator::init(void *pAddr)
{
	_pHead = static_cast<tagSpanAlloc*>(pAddr);
	_pData = (unsigned char*)((char*)_pHead + sizeof(tagSpanAlloc));
}

void NF_SpanAllocator::create(void* pAddr, size_t iSpanSize, size_t iSpanCount)
{
...
void NF_SpanAllocator::connect(void *pAddr)
{
	init(pAddr);
}

void* NF_SpanAllocator::allocate()
{
...
void NF_SpanAllocator::deallocate(void *pAddr)
{
	assert(pAddr >= _pData);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void NF_Page::DLL_Init(TC_Span* list, size_t iIndex)
{
	list->next = iIndex;
	list->prev = iIndex;
}
...
void NF_Page::DLL_Remove(TC_Span *span)
{
...
void NF_Page::DLL_Prepend(TC_Span *list, TC_Span *span)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
NF_Page::TC_Span* NF_Page::SearchFreeAndLargeLists(size_t n)
{
...
NF_Page::TC_Span* NF_Page::New(size_t n)
{
...
NF_Page::TC_Span* NF_Page::AllocLarge(size_t n)
{
	TC_Span *best = NULL;
	size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
...
NF_Page::TC_Span* NF_Page::Carve(TC_Span *span, size_t n)
{
...
void NF_Page::MergeIntoFreeList(TC_Span* span)
{
...
void NF_Page::RegisterSizeClass(TC_Span *span, size_t sc)
{
	span->sizeclass = sc;
...
NF_Page::TC_Span* NF_Page::GetDescriptor(size_t pPageId)
{
...
void NF_Page::RecordSpan(TC_Span *span)
{
	Set(span->start, reinterpret_cast<size_t>(span) - reinterpret_cast<size_t>(_spanAlloc.getHead()));
	if (span->length > 1)
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void* NF_Page::fetchFromSpansSafe(size_t iClassSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
{
...
void* NF_Page::FetchFromSpans(size_t iClassSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
{
	size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
	...
	size_t _size = NF_Static::sizemap()->ByteSizeForClass(iClassSize);
	iPageId = span->start;
...
int NF_Page::Populate(size_t iClassSize)
{
	const size_t npages = NF_Static::sizemap()->class_to_pages(iClassSize);
	bool      flag = _pShmFlagHead->_bShmProtectedArea;
	...
	const  size_t size = NF_Static::sizemap()->ByteSizeForClass(iClassSize);
...
void NF_Page::releaseToSpans(size_t iPageId, size_t iIndex)
{
	...
	{
		void*  ptr = reinterpret_cast<void*>(iBeginAddr + _pShmFlagHead->_iShmPageAddr + (iPageId << kPageShift) + iIndex *  NF_Static::sizemap()->ByteSizeForClass(_size_class));
		if (flag)
		{
...
void  NF_Page::releaseToSpans(void* pObject)
{
	size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
...
void* NF_Page::getAbsolute(size_t iPageId, size_t iIndex)
{
	TC_Span* span = GetDescriptor(iPageId);
	assert(span != NULL);
	const size_t _size_class = span->sizeclass;
	...
	return reinterpret_cast<void*>(reinterpret_cast<size_t>(_pShmFlagHead) + _pShmFlagHead->_iShmPageAddr + (iPageId << kPageShift) + iIndex *  NF_Static::sizemap()->ByteSizeForClass(_size_class));
}

void NF_Page::doUpdate(bool bUpdate)
{
	if (bUpdate)
	{
...
void NF_Page::initPage(void *pAddr)
{
	...
	_pSpanMemHead = static_cast<NF_SpanAllocator*>((void*)((char*)span_begin_ptr));
	_pShmFlagHead->_iShmSpanAddr = end - begin;

	size_t _iCapacitySize = _pShmFlagHead->_iShmMemSize - (end - begin);
	size_t _iPagesNum = (_iCapacitySize - NF_SpanAllocator::getHeadSize() - sizeof(TC_Span) * 20) / (sizeof(TC_Span) + sizeof(size_t) + kPageSize);

	size_t*    pagemap_ptr = reinterpret_cast<size_t*>((char*)_pSpanMemHead + NF_SpanAllocator::getHeadSize() + sizeof(TC_Span) * (_iPagesNum + 20));
...
void NF_Page::create(void *pAddr, size_t iShmMemSize, bool bProtectedArea)
{
	init(pAddr);
...
void NF_Page::connect(void *pAddr)
{
	init(pAddr);
	size_t iBeginAddr = reinterpret_cast<size_t>(pAddr);

	_pSpanMemHead = reinterpret_cast<NF_SpanAllocator*>(iBeginAddr + _pShmFlagHead->_iShmSpanAddr);
...
size_t NF_Page::getPageNumber()
{
	return _pShmFlagHead->_iShmPageNum;
}

size_t NF_Page::getPageMemSize()
{
	return _pShmFlagHead->_iShmPageNum << kPageShift;
}

size_t NF_Page::getPageMemEnd()
{
	return reinterpret_cast<size_t>(_pShmFlagHead) + _pShmFlagHead->_iShmMemSize;
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void NFMallocChunkAllocator::init(void *pAddr)
{
	_pHead = static_cast<tagChunkAllocatorHead*>(pAddr);
	_pChunk = (char*)_pHead + sizeof(tagChunkAllocatorHead);
}

void NFMallocChunkAllocator::create(void *pAddr, size_t iSize, bool bProtectedArea)
{
	assert(iSize > (sizeof(tagChunkAllocatorHead) + NF_Page::getMinMemSize()));
	init(pAddr);
...
void NFMallocChunkAllocator::_connect(void *pAddr)
{
	clear();
	...
	_nallocator = new NFMallocChunkAllocator();
	_nallocator->connect(pNextHead);
}

void NFMallocChunkAllocator::connect(void *pAddr)
{
	_connect(pAddr);
	doUpdate();
...
void NFMallocChunkAllocator::rebuild()
{
	_page.rebuild();
...
NFMallocChunkAllocator * NFMallocChunkAllocator::lastAlloc()
{
	if (_nallocator == NULL)
		return NULL;

	NFMallocChunkAllocator *p = _nallocator;
...
void NFMallocChunkAllocator::append(void *pAddr, size_t iSize)
{
	connect(pAddr);
	...
	NFMallocChunkAllocator *p = new NFMallocChunkAllocator();
	p->create(pAppendAddr, iSize - _pHead->_iTotalSize, _pHead->_bProtectedArea);

	NFMallocChunkAllocator *palloc = lastAlloc();
	if (palloc)
	{
...
void* NFMallocChunkAllocator::allocate(size_t iNeedSize, size_t &iAllocSize)
{
	size_t iPageId = 0;
	size_t iIndex = 0;
...
void  NFMallocChunkAllocator::deallocate(void* pAddr)
{
	if (_nallocator)
	{
...
void* NFMallocChunkAllocator::allocate(size_t iNeedSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
{
	...
	size_t iClassSize = NF_Static::sizemap()->SizeClass(iNeedSize);
	void* p = NULL;
...
void NFMallocChunkAllocator::deallocate(size_t iPageId, size_t iIndex)
{
	if (_nallocator)
	{
		size_t prev = _page.getPageNumber();
...
void* NFMallocChunkAllocator::getAbsolute(size_t iPageId, size_t iIndex)
{
	if (_nallocator == NULL)
	{
...
size_t NFMallocChunkAllocator::getAllCapacity()
{
...
void   NFMallocChunkAllocator::getSingleCapacity(std::vector<size_t> &vec_capacity)
{
	vec_capacity.clear();
...
void NFMallocChunkAllocator::doUpdate(bool bUpdate)
{

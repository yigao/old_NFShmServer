...
void NFMemQueue::create(void *pAddr, size_t iSize)
{
	assert(pAddr);
	assert(iSize != 0);
...
void NFMemQueue::connect(void *pAddr, size_t iSize)
{
	assert(pAddr);
...
bool NFMemQueue::isFull(size_t iSize)
{
	doUpdate();
...
bool NFMemQueue::isEmpty()
{
	doUpdate();
	return (_pctrlBlock->iTopIndex == _pctrlBlock->iBotIndex);
}

size_t NFMemQueue::queueSize()
{
	doUpdate();
	return _size - sizeof(CONTROL_BLOCK) - sizeof(tagModifyHead);
}

size_t NFMemQueue::elementCount()
{
	doUpdate();
...
bool NFMemQueue::push_back(const std::string &sIn)
{
	return push_back(sIn.c_str(), sIn.length());
}

bool NFMemQueue::push_back(const char *pvIn, size_t iSize)
{
	if (isFull(iSize))
	{
		return false;
...
void NFMemQueue::doUpdate(bool bUpdate)
{
	if (bUpdate)
	{
...
void NFMemQueue::update(void* iModifyAddr, size_t iModifyValue)
{
...
void NFMemQueue::update(void* iModifyAddr, bool bModifyValue)
{
	_pstModifyHead->_cModifyStatus = 1;
	_pstModifyHead->_stModifyData[_pstModifyHead->_iNowIndex]._iModifyAddr = (char*)iModifyAddr - (char*)_pctrlBlock;

...
{
public:
	static NF_SizeMap* sizemap()
	{
		return &_sizemap;
	}
private:
	static NF_SizeMap _sizemap;
};
...
*/
class NF_SpanAllocator
{
public:
	NF_SpanAllocator() : _pHead(NULL), _pData(NULL) {}
...
protected:
	...
	*/
	...
	//禁止copy构造
	NF_SpanAllocator(const NF_SpanAllocator &mcm);

	//禁止复制
	NF_SpanAllocator &operator=(const NF_SpanAllocator &mcm);
...
#endif
public:
	NF_Page() : _pShmFlagHead(NULL), _pModifyHead(NULL), _pCenterCache(NULL), _pLarge(NULL), _pFree(NULL), _pSpanMemHead(NULL), _pPageMap(NULL), _pData(NULL) {}
	...
	*/
	...
	{
		return sizeof(tagShmFlag) + sizeof(tagModifyHead) + sizeof(TC_CenterList) * kNumClasses + sizeof(TC_Span) + sizeof(TC_Span) * kMaxPages + NF_SpanAllocator::getHeadSize() + sizeof(TC_Span) + sizeof(size_t) + kPageSize;
	}

protected:

	//禁止copy构造
	NF_Page(const NF_Page &mcm);

	//禁止复制
	NF_Page &operator=(const NF_Page &mcm);
...
private:
	...
	*/
	NF_SpanAllocator          *_pSpanMemHead;
	/**
	* 与数据区内存相映射的内存区头指针
	*/
	//void                      **_pPageMap;
	...
	* 用于分配TC_Span的内存分配器
	*/
	NF_SpanAllocator          _spanAlloc;
};

////////////////////////////////////////////////////////////////////////////////////////////////////
class NFMallocChunkAllocator
{
public:
	NFMallocChunkAllocator() :_pHead(NULL), _pChunk(NULL), _nallocator(NULL) {}

	~NFMallocChunkAllocator()
	{
		clear();
	}
...
#pragma pack(pop)
	...
	*/
	static size_t getNeedMinSize() { return sizeof(tagChunkAllocatorHead) + NF_Page::getMinMemSize(); }
protected:
	void init(void *pAddr);

	void _connect(void *pAddr);

	NFMallocChunkAllocator *lastAlloc();

	//禁止copy构造
	NFMallocChunkAllocator(const NFMallocChunkAllocator &);

	//禁止复制
	NFMallocChunkAllocator& operator=(const NFMallocChunkAllocator &);
private:
	/**
	* 头指针
	*/
	...
	* 后续的多块分配器
	*/
	NFMallocChunkAllocator *_nallocator;
};

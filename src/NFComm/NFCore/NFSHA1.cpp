...
#define rnd(f,k)    \
	...
	/* NFPebbleSha1 hash data in an array of bytes into hash buffer and call the        */
	...
	/* NFPebbleSha1 final padding and digest calculation  */
...
std::vector<char> NFSHA1::sha1bin(const char *data, size_t len)
{
...
std::string NFSHA1::sha1str(const char *buffer, size_t length)
{
	std::vector<char> digest = sha1bin(buffer, length);

	return NFCommon::bin2str(&digest[0], digest.size());
}

std::string NFSHA1::sha1str(const std::string& buffer)
{
    return sha1str(buffer.data(), buffer.length());
}

std::string NFSHA1::sha1file(const std::string &fileName)
{
	...
	if ((f = fopen(fileName.c_str(), "rb")) == NULL)
		throw NF_SHA_Exception("[TC_SHA::sha1file] fopen '" + fileName + "', error.", errno);
	...
	return NFCommon::bin2str(sOutBuffer, sizeof(sOutBuffer));
}

std::vector<char> NFSHA1::sha256bin(const char *data, size_t len)
{
	std::vector<char> digest;
...
std::string NFSHA1::sha256str(const char *buffer, size_t length)
{
	std::vector<char> digest = sha256bin(buffer, length);

	return NFCommon::bin2str(&digest[0], digest.size());
}

std::string NFSHA1::sha256file(const std::string &fileName)
{
	...
	detail_sha2::sha256_ctx cx[1];
	if ((f = fopen(fileName.c_str(), "rb")) == NULL)
		throw NF_SHA_Exception("[TC_SHA::sha256file] fopen '" + fileName + "', error.", errno);
...
std::vector<char> NFSHA1::sha384bin(const char *data, size_t len)
{
	std::vector<char> digest;
...
std::string NFSHA1::sha384str(const char *buffer, size_t length)
{
...
std::string NFSHA1::sha384file(const std::string &fileName)
{
	...
	detail_sha2::sha384_ctx cx[1];
	if ((f = fopen(fileName.c_str(), "rb")) == NULL)
		throw NF_SHA_Exception("[TC_SHA::sha384file] fopen '" + fileName + "', error.", errno);
...
std::vector<char> NFSHA1::sha512bin(const char *data, size_t len)
{
	std::vector<char> digest;
...
std::string NFSHA1::sha512str(const char *buffer, size_t length)
{
	std::vector<char> digest = sha512bin(buffer, length);

	return NFCommon::bin2str(&digest[0], digest.size());
}

std::string NFSHA1::sha512file(const std::string &fileName)
{
	...
	detail_sha2::sha512_ctx cx[1];
	if ((f = fopen(fileName.c_str(), "rb")) == NULL)
		throw NF_SHA_Exception("[TC_SHA::sha512file] fopen '" + fileName + "', error.", errno);

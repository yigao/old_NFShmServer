// -------------------------------------------------------------------------
//    @FileName         :    NFZookeeperCache.cpp
//    @Author           :    xxxxx
//    @Date             :   xxxx-xx-xx
//    @Email			:    xxxxxxxxx@xxx.xxx
//    @Module           :    NFZookeeperCache.cpp
//
// -------------------------------------------------------------------------
...
NFZookeeperCache::NFZookeeperCache(NFZookeeperClient* zookeeper_client)
...
NFZookeeperCache::~NFZookeeperCache()
{
}

int32_t NFZookeeperCache::SetConfig(int32_t refresh_time_ms, int32_t invalid_time_ms)
{
...
int32_t NFZookeeperCache::Get(const std::string& name, std::vector<std::string>* urls)
{
...
void NFZookeeperCache::OnDelete(const std::string& name)
{
...
void NFZookeeperCache::OnGetCb(int rc, GetAllNameResults& results, const NFNamingCbReturnValue& cb) // NOLINT
{
    if (cb == NULL) {
        return;
    }
...
void NFZookeeperCache::OnUpdateCb(const std::string& req_name, bool is_watch,
                                  int rc, GetAllNameResults& results, NFNamingCbReturnValue cb) // NOLINT
{
    // 加入到定时更新中
    if ((proto_ff::ERR_CODE_ZK_NOAUTH != rc && proto_ff::ERR_CODE_ZK_BADARGUMENTS != rc)
        && NULL != cb
...
void NFZookeeperCache::OnWatchCb(const std::string& req_name,
...
void NFZookeeperCache::Update()
{
    ...
    if (m_curr_time > m_last_refresh + m_refresh_time_ms) {
        ...
        for ( ; it != m_cache_keys.end() ; ++it) {
            NFNamingCbReturnValue null_cob = NULL;
            CbGetAllReturn get_cb = std::bind(&NFZookeeperCache::OnUpdateCb,
                                              this, it->first, it->second, std::placeholders::_1, std::placeholders::_2, null_cob);
...
void NFZookeeperCache::UpdateCache(GetAllNameResults& results, bool is_watch) // NOLINT
{
    // 更新cache和回调通知
